  ...
  for (s of json) {
    $("#version_log").append(
      s.text == json[json.length - 1].text
        ? `<h3>${s.version}   <span class ="badge badge-secondary"> New </span></h3><p>${s.text}</p><p class='text-right text-secondary '>${s.datetime}</p>`
        : `<h3>${s.version}</h3><p>${s.text}</p><p class='text-right text-secondary '>${s.datetime}</p>`
    );
  }
});
...
$(document).ready(function () {
  ...
  $("#github").html(
    '<a href="https://github.com/YoungDaggerDee/ToDoApp" target="popup"> Github </a>'
  );
});
